...
archival_object_csv = os.path.normpath("subjects.csv")
...
with open(archival_object_csv, 'r') as csvfile:
    ...
    for row in reader:
        ...
        repo_subject = row[1]  # column 2 in CSV, first column is column 0
        # Grab the archival object's ArchivesSpace ref_id from the CSV
        component_id = row[0]  # column 1
        logging.info("ROW: " + str(n) + " SUBJECT " + str(repo_subject) + " COMPONENT " + str(component_id))
        ...
        if archival_object_json['component_id'] == component_id:
            ...
            subject_ref = {"ref":"/subjects/" + repo_subject}
            archival_object_json['subjects'].append(subject_ref)
